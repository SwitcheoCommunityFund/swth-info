...
$table_name = 'rewards';
$message_types = ['claim_pool_rewards','stake_pool_token'];

$res = $db->query("select coalesce(max(id),0) as last_id from \"$table_name\" ")->fetch(\PDO::FETCH_OBJ);
$last_id = $res->last_id;
$res = $db->query("select coalesce(max(id),0) as last_id from \"$table_name\" where tr_type in ('" . implode("','",$message_types) . "')")->fetch(\PDO::FETCH_OBJ);
...
$params = [
    'msg_type'=>implode(',',$message_types),
    'before_id'=>$last_id,
    'after_id'=>$after_id,
    'limit'=>$api->api_limit,
    'order_by'=>'asc'
...
do {
    ...
    foreach ($trs as $tr)
    {
        ...
        if (!array_key_exists($msg_index,$previous_tr_logs['logs'])){
            var_dump($tr->hash,$previous_tr_logs,$logs_raw,$logs,$tr,'tr_id='.(@$tr->id));
