...
<div class="col-md-12" id="charts_placement">
    <?php
...
foreach ($trades_summary as $summary)
{
    ...
    $denom_shrt = mb_strimwidth(trim($summary['denom'],1),0,6,'..');
    $denom_lng = trim($summary['denom'],1);

    $total_line       .= '<td align="right" class="focusout">' . ($k==1?'&nbsp;(':'') . prettyNum($summary['trade_sum'])       . '<span class="denom" title="'.$denom_lng.'">'. $denom_shrt . "</span>" . ($pls==null?')':'') . "&nbsp;{$pls}&nbsp;</td>";
    $total_24h_line   .= '<td align="right" class="focusout">' . ($k==1?'&nbsp;(':'') . prettyNum($summary['trade_24h_sum'])   . '<span class="denom" title="'.$denom_lng.'">'. $denom_shrt . "</span>" . ($pls==null?')':'') . "&nbsp;{$pls}&nbsp;</td>";
    $total_month_line .= '<td align="right" class="focusout">' . ($k==1?'&nbsp;(':'') . prettyNum($summary['trade_month_sum']) . '<span class="denom" title="'.$denom_lng.'">'. $denom_shrt . "</span>" . ($pls==null?')':'') . "&nbsp;{$pls}&nbsp;</td>";
    $k++;
}
